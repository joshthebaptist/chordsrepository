...
import { Song, SongSection } from "@/lib/types";
...
export default function SongDetailPage() {
  ...
  useEffect(() => {
    fetch(`/api/songs/${id}`)
      .then((r) => r.json())
      .then((data) => {
        if (data.error) { router.push("/songs"); return; }
        setSong(data);
        setTitle(data.title);
  ...
  const handleSave = async () => {
    if (!title.trim()) { alert("Please enter a song title."); return; }
    setSaving(true);
    try {
      const res = await fetch(`/api/songs/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: title.trim(), sections, currentKey, editedBy: editedBy.trim() }),
      });
      if (res.ok) {
    ...
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center justify-between gap-4">
        <div className="flex-1">
          ...
          <h1 className="font-display text-3xl font-bold text-stone-800">Edit Song</h1>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="text" value={editedBy} onChange={(e) => setEditedBy(e.target.value)}
            className="w-full px-4 py-2.5 bg-white border border-stone-200 rounded-xl text-sm focus:border-gold focus:ring-2 focus:ring-gold/20 outline-none transition-all" />
        </div>
      </div>
